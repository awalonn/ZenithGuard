...
/**
 * Updates all enabled filter lists, respecting cache duration.
...
export async function updateAllLists(force = false) {
    ...
    if (updatePromises.length > 0) {
        console.log(`ZenithGuard: Updating ${updatePromises.length} filter list(s)...`);
...
export async function getHidingRulesForDomain(domain) {
    const { filterLists = [], isPerformanceModeEnabled } = await chrome.storage.sync.get(['filterLists', 'isPerformanceModeEnabled']);
    if (isPerformanceModeEnabled) return { rules: [] };
    
    let allRulesForDomain = [];
    const enabledLists = (filterLists || []).filter(l => l.enabled && l.status === 'success');
